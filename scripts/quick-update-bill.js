...
const CONFIG = {
    // 直接访问资金中心页面的URL
    planDirectUrl: 'https://mc.pinduoduo.com/ddmc-mms/finance-return-page?jumpType=financeCenter',
    // 登录后跳转到资金中心页面的URL
    planLoginUrl: 'https://mms.pinduoduo.com/login/?redirectUrl=https://mc.pinduoduo.com/ddmc-mms/finance-return-page?jumpType=financeCenter',
    // 目标API端点
    targetApiEndpointPlan: 'finance/account/queryAccountLoginInfo?__app_code=116',
...
class PDDPlanAntiContentFetcher {
    constructor(loginCredentials, userDataDir, supabaseClient) {
        ...
        this.capturedData = {
            antiContent: null,
            antiContentPlan: null, // 资金中心专用的anti-content参数
        ...
        this.page.on('request', async (request) => {
            ...
            if (url.includes(CONFIG.targetApiEndpointPlan)) {
                ...
                if (headers['anti-content']) {
                    ...
                    console.log('   ✅ 捕获到资金中心的anti-content:', this.capturedData.antiContent);
                }
                if (headers['cookie']) {
                    this.capturedData.antiContentPlan = headers['cookie'];
                    
                    console.log('   ✅ 捕获到资金中心专用的cookie:', this.capturedData.antiContentPlan);
                }
    ...
    async waitForPlanAPIRequest() {
        ...
        if (this.capturedData.antiContent) {
            console.log(`✅ 已捕获到anti-content，直接返回（长度: ${this.capturedData.antiContent.length}）`);
            return true;
        }
        ...
        try {
            ...
            console.log(`✅ 已捕获到资金中心查询API请求，获取到anti-content（长度: ${this.capturedData.antiContent.length}）`);
            return true;
        } catch (error) {
            console.log('❌ 在30秒内未捕获到资金中心查询API请求');
            return false;
        }
    }
...
async function updatePlanAntiContent(username, password) {
    ...
    try {
        ...
        if (fetcher.capturedData.antiContent && !fetcher.capturedData.needlogin) {
            // 只更新anti_content字段
            const { error } = await supabase
                .from('pdd_accounts')
                .update({
                  anti_content: fetcher.capturedData.antiContent,
                  anti_content_Plan: fetcher.capturedData.antiContentPlan, // 更新资金中心专用的anti-content
        ...
        } else if (fetcher.capturedData.antiContent && fetcher.capturedData.needlogin){
